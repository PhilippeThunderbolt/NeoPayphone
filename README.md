...
- Replaced sound board with **Adafruit AudioFX** (16 MB max size), USB updatable.
- Added external jack and speaker to increase ring loudness (amplifier volume knob still works).
- Drilled a larger hole in the back case to allow power and speaker cables to pass through easily.
- Reduced lag in keypad entry.
- Added more error messages to the LCD screen to aid troubleshooting.
- Increased number of incoming calls to **8** by requiring two button presses on the remote (e.g., **AA**, **AB**).  
- The button sequence is displayed on the LCD as it is entered to assist with troubleshooting.
- Smoother overall operation.
- Optimized memory usage — the **Arduino UNO** is constrained to ~2 KB of SRAM, so all file names are numeric and stored as integers rather than strings.
- Rewrote the code as a **state machine** to make understanding and maintenance much easier. Anyone familiar with Arduino code should be able to extend or modify it.
- Connected the sound board using pin wires taped in place to allow future microcontroller replacement.
...
1. **OGG** — Dial tone sound  
   Plays when the handset is lifted to dial a number.

2. **WAV** — Button press sound  
   Plays when a keypad button is pressed.

3. **OGG** — Ringing tone for incoming call  
   Plays in two places:
...
4. **OGG** — Call finished sound  
   Plays:
   - At the end of a dialed number
   - If a bad number is dialed
...
Outbound phone numbers are 5 numeric digits, and can be in OGG or WAV (OGG is smaller).  There can be a MAXIMUM of 18 of these files, and any more will be ignored.e.g.
